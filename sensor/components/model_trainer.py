...
class ModelTrainer:
    ...
    def train_model(x, y, xgb_params=None):
        try:
            xgb_clf = XGBClassifier(**(xgb_params or {}))
            xgb_clf.fit(x, y)
            return xgb_clf
        except Exception as e:
            raise SensorException(e, sys)
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        try:
            ...
            logging.info(f"Train the model with XGB params from config: {self.model_trainer_config.xgb_params}")
            model = ModelTrainer.train_model(
                x=x_train,
                y=y_train,
                xgb_params=self.model_trainer_config.xgb_params
            )
            ...
            if f1_test_score < self.model_trainer_config.expected_score:
                raise Exception(
                    f"Model is not good as it is not able to give expected accuracy: "
                    f"{self.model_trainer_config.expected_score}: model actual score: {f1_test_score}"
                )
            ...
            if diff > self.model_trainer_config.overfitting_threshold:
                raise Exception(
                    f"Train and test score diff: {diff} is more than overfitting threshold "
